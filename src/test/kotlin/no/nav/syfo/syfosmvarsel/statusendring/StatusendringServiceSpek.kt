    ...
    FunSpec({
        ...
        context("Test av statusendring") {
            test("handterStatusendring ferdigstiller brukernotifikasjon hvis sykmelding er SENDT") {
                ...
                verify(exactly = 1) {
                    brukernotifikasjonServiceMock.ferdigstillBrukernotifikasjon(
                        sykmeldingStatusKafkaMessageDTO
                    )
                }
            }
            ...
            test(
                "handterStatusendring ferdigstiller brukernotifikasjon hvis sykmelding er BEKREFTET"
            ) {
                val sykmeldingStatusKafkaMessageBekreftet =
                    SykmeldingStatusKafkaMessageDTO(
                        event =
                ...
                verify(exactly = 1) {
                    brukernotifikasjonServiceMock.ferdigstillBrukernotifikasjon(
                        sykmeldingStatusKafkaMessageBekreftet
                    )
                }
            }

            test(
                "handterStatusendring ferdigstiller brukernotifikasjon hvis sykmelding er AVBRUTT"
            ) {
                val sykmeldingStatusKafkaMessageAvbrutt =
                    SykmeldingStatusKafkaMessageDTO(
                        event =
                            sykmeldingStatusKafkaMessageDTO.event.copy(
                ...
                verify(exactly = 1) {
                    brukernotifikasjonServiceMock.ferdigstillBrukernotifikasjon(
                        sykmeldingStatusKafkaMessageAvbrutt
                    )
                }
            }

            test(
                "handterStatusendring ferdigstiller ikke brukernotifikasjon hvis sykmelding er APEN"
            ) {
                val sykmeldingStatusKafkaMessageApen =
                    SykmeldingStatusKafkaMessageDTO(
                        event =
                            sykmeldingStatusKafkaMessageDTO.event.copy(statusEvent = STATUS_APEN),
                        kafkaMetadata = sykmeldingStatusKafkaMessageDTO.kafkaMetadata,
                    )
                ...
                verify(exactly = 0) {
                    brukernotifikasjonServiceMock.ferdigstillBrukernotifikasjon(any())
                }
            }
        }
    })
